package com.example.abner.slidelistview.slidelistview;
...
import com.example.abner.slidelistview.slidelistview.wrap.FrontViewWrapLayout;
import com.example.abner.slidelistview.slidelistview.wrap.SlideItemWrapLayout;
import com.example.abner.slidelistview.slidelistview.SlideListView.*;
...
public class SlideTouchListener implements OnTouchListener {
    ...
    private static final String TAG = SlideTouchListener.class.getSimpleName();

    private static final int DIRECT_STATE_NONE = 0;// no sliding
    private static final int DIRECT_STATE_LEFT = 1;// manual sliding
    private static final int DIRECT_STATE_RIGHT = 2;// auto sliding
    ...
    //0 isInit 1 left  2 right
    private int mDirect;

    private int mDefalutColor = Color.parseColor("#e73a3d");

    private int mLeftNomalColor = Color.parseColor("#67D95A");

    private int mCancelColor = Color.parseColor("#E3E3E3");

    private int mLeftSelectedColor = Color.parseColor("#E44E0B");

    private int mCancelX ;

    private int mLeftSelectedX;
    ...
    private class SlideItem {
        ...
        public SlideItem(int pos) {
            ...
            child.setBackgroundColor(Color.parseColor("#e73a3d"));
            if (child == null) {
            ...
            mCancelX = child.getWidth() >> 3;
            mLeftSelectedX = child.getWidth() >> 1;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {// Handle the event which dispatch to
                ...
                if (mScrollState == SLIDING_STATE_MANUAL) {
                    ...
                    if (deltaX - mSlideItem.preDelatX > 0) {
                        mDirect = DIRECT_STATE_RIGHT;
                    } else if (deltaX - mSlideItem.preDelatX < 0) {
                        mDirect = DIRECT_STATE_LEFT;
                    }
            ...
            case MotionEvent.ACTION_UP: {
				 ...
				 */
                    ...
                    if (mSlideItem.offset > 0) {// left back view is showing
                        if (slideMode == SlideMode.LEFT || slideMode == SlideMode.BOTH) {// SlideMode
                            ...
                            boolean distanceGreater = Math.abs(mSlideItem.offset - mSlideItem.previousOffset) > Math.abs(mSlideItem.maxOffset)
                                    / (float) 4;
                            Log.d(TAG, " left mSlideItem.offset : " + mSlideItem.offset + " mSlideItem.previousOffset : " + mSlideItem.previousOffset + " distanceGreater " + distanceGreater);
                        ...
                        if (slideMode == SlideMode.RIGHT || slideMode == SlideMode.BOTH) {// SlideMode
                            ...
                            boolean distanceGreater = Math.abs(mSlideItem.offset - mSlideItem.previousOffset) > Math.abs(mSlideItem.minOffset)
                                    / (float) 4;
                            Log.d(TAG, " right  mSlideItem.offset : " + mSlideItem.offset + " mSlideItem.previousOffset : " + mSlideItem.previousOffset + " distanceGreater " + distanceGreater);
                            if (mSlideItem.offset - mSlideItem.previousOffset > 0) {
                                doOpen = !distanceGreater;
                            } else {
                                doOpen = distanceGreater;
                    ...
                    Log.d(TAG, " direct : " + mDirect);
                    if(doOpen){
                        if(mSlideItem.offset > 0 && mDirect == DIRECT_STATE_LEFT){
                            doOpen = false;
                        }
                        if(mSlideItem.offset < 0 && mDirect == DIRECT_STATE_RIGHT){
                            doOpen = false;
                        }

                        if(Math.abs(mSlideItem.offset) < mLeftSelectedX){
                            doOpen = false;
                        }
    ...
    private void slidingFinish() {
        ...
        mSlideItem.child.setBackgroundColor(mDefalutColor);
        mSlideItem.child.setSelected(false);
        Log.d(TAG," mSlideItem.offset " + mSlideItem.offset);
        if (mSlideItem.offset != 0) {
            ...
            if (mSlideItem.child.getScrollDeleteCallBack() != null) {
                mSlideItem.child.getScrollDeleteCallBack().onDelete(mSlideItem.child);
            }
        } else {
    ...
    private void autoScroll(final int offset, final boolean toOpen) {
        ...
        int moveTo = 0;
        Log.d(TAG, " autoScroll : " + offset + " toOpen " + toOpen);
    ...
    private void move(int offset) {
        ...
        if (offset < 0) {// offset less than 0,right back view is showing and
            ...
            //process left
            if(Math.abs(offset) < mCancelX){
                mSlideItem.child.setBackgroundColor(mCancelColor);
                mSlideItem.child.getLeftBackView().setSelected(false);
            } else if(Math.abs(offset) > mCancelX && Math.abs(offset) < mLeftSelectedX){
                mSlideItem.child.setBackgroundColor(mLeftNomalColor);
                mSlideItem.child.getLeftBackView().setSelected(false);
            } else if(Math.abs(offset) > mLeftSelectedX){
                mSlideItem.child.setBackgroundColor(mLeftSelectedColor);
                mSlideItem.child.getLeftBackView().setSelected(true);
            }
        }
    }
